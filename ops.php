...
Description: On Page SEO Robots.txt plugin helps you boosting your website indexation and your ranking by adding specific instructions in your robots.txt, sitemap.xml file and Canonical tag  which added automatically in your website just adding this plugin. No need to have coding (tech) skills to use OPS - Robots.txt plugin.
Version:     1.0.0
...
require_once OPSRBT_PLUGIN_PATH . 'functions.php';
//require_once OPSRBT_PLUGIN_PATH . 'uninstall.php';
require_once OPSRBT_PLUGIN_PATH . 'shortcode.php';

if ( ! function_exists( 'opsrbt_styles' ) ) {
function opsrbt_styles () 
{
    if ( is_admin() ) {
    	wp_enqueue_style( 'opsrbt_bs_css', plugins_url('css/bootstrap.min.css', __FILE__) );
    	wp_enqueue_style( 'opsrbt_custom_css', plugins_url('css/custom.css', __FILE__) );  
    	wp_enqueue_script( 'opsrbt_bs_js', plugins_url( 'js/popper.min.js', __FILE__ ));
        wp_enqueue_script( 'opsrbt_bmj_js', plugins_url( 'js/bootstrap.min.js', __FILE__ ));
    	wp_enqueue_script( 'opsrbt_bndl_js', plugins_url( 'js/bootstrap.bundle.min.js', __FILE__ ));
    }
}  
}
